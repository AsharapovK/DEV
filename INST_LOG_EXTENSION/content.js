...
function sendStatusToBackground(isOnline, message = '') {
    chrome.runtime.sendMessage({
        action: 'updateStatus',
        isOnline,
        message: message || (isOnline ? 'Обнаружена активность' : 'Активность не обнаружена')
    }).catch(error => {
...
function checkForActivity() {
    try {
        ...
        for (const el of elements) {
            if (el.classList.contains("xbe7ycp")) {
                if (!activityDetected) {
...
function startActivityCheck() {
    if (!checkInterval) {
        ...
        const fastCheckInterval = setInterval(() => {
            if (isExtensionActive && window.location.href.includes('instagram.com')) {
                checkForActivity();
            }
        }, 1000);
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    if (request.action === 'checkStatus') {
        if (!window.location.href.includes('instagram.com')) {
            const errorMsg = 'Not on Instagram';
